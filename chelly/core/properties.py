...
class Properties(object):
    ...
    def indent_with_tabs(self, tabs:bool) -> None:
        if tabs:
            ...
            self.__set_tab_distance("W", self.indent_size)
        else:
            self._indent_type = Property.Indentation.spaces
            self.__set_tab_distance(" ", self.indent_size)
    ...
    def indent_size(self, size:int) -> None:
        ...
        if self.indent_with_spaces:
            self.__set_tab_distance(" ", size)
        else:
            self.__set_tab_distance("W", size)
    ...
    def __set_tab_distance(self, char:str, indent_size:int):
        ...
        assert bearing_left >= 0, "BEARING MUST BE MORE THAN 0"
        assert bearing_right >= 0, "BEARING MUST BE MORE THAN 0"
