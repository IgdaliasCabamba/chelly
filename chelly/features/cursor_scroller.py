...
class CursorScroller(Feature):
    ...
    def current_line_number_down(self) -> int:
        return (
            TextEngine(self.editor).current_line_nbr + CursorScroller.LINE_NUMBER_DELTA
        )
    ...
    def natural_scroll_limit(self):
        return (TextEngine(self.editor).line_count + 5) - (
            TextEngine(self.editor).amount_of_visible_blocks // 2
        )
